...
function AlejandroPaezPage({ isEditMode = false }) {
  ...
  const handleDeleteProject = (projectId, projectName) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        alejandroProjectManager.deleteProject(projectId);
        loadProjects();
  ...
  const handleDeleteSection = (sectionId, sectionName) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        alejandroProjectManager.deleteSection(sectionId);
        loadSections();
        loadProjects();
  ...
  const handleSaveSection = () => {
    ...
    if (editingSection) {
      alejandroProjectManager.updateSection(editingSection.id, sectionFormData);
      Swal.fire('Actualizado', 'La sección ha sido actualizada.', 'success');
    } else {
      alejandroProjectManager.saveSection(sectionFormData);
