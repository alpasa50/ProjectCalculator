...
import { alejandroProjectManager } from '../utils/alejandroProjectManager';
import Swal from 'sweetalert2';
import '../utils/PaymentCalc.css'; // Reuse CSS for styling

function AlejandroPaezPage({ isEditMode = false }) {
  ...
  const loadSections = useCallback(async () => {
    const allSections = await alejandroProjectManager.getAllSections();
    setSections(allSections);
  }, []);

  const loadProjects = useCallback(async () => {
    const allProjects = await alejandroProjectManager.getAllProjects();
    const normalizedProjects = await Promise.all(allProjects.map(async (p) => {
      ...
      if (status !== project.status) {
        await alejandroProjectManager.updateProject(project.id, { ...project, status });
      }
      return {
        ...project,
        status
  ...
  useEffect(() => {
    const projectSubscription = alejandroProjectManager.subscribeToProjectsChanges(async () => {
      await loadProjects();
    });
    const sectionSubscription = alejandroProjectManager.subscribeToSectionsChanges(async () => {
      await loadSections();
    });
  ...
  const handleDeleteProject = async (projectId, projectName) => {
    ...
    if (result.isConfirmed) {
      await alejandroProjectManager.deleteProject(projectId);
      await loadProjects();
  ...
  const handleDeleteSection = async (sectionId, sectionName) => {
    ...
    if (result.isConfirmed) {
      await alejandroProjectManager.deleteSection(sectionId);
      await loadSections();
      await loadProjects();
  ...
  const handleSaveSection = async () => {
    ...
    if (editingSection) {
      await alejandroProjectManager.updateSection(editingSection.id, sectionFormData);
      Swal.fire('Actualizado', 'La sección ha sido actualizada.', 'success');
    } else {
      await alejandroProjectManager.saveSection(sectionFormData);
  ...
  const handleSaveProject = async () => {
    ...
    try {
      if (editingProject) {
        await alejandroProjectManager.updateProject(editingProject.id, projectData);
        Swal.fire('Actualizado', 'El proyecto ha sido actualizado.', 'success');
      } else {
        await alejandroProjectManager.saveProject(projectData);
  ...
  return (
    <div className="calculator-container alejandro-page">
      <h1>Proyectos de Alejandro Paez</h1>
      {isEditMode && (
...
export default AlejandroPaezPage;